...
def get_recommendations(user_id):
    ...
    # ✅ Step 8: Combine All Recommendations but Remove Duplicates BEFORE Final Selection
    ...
    # ✅ Step 9: Remove Previously Recommended Videos from Session
    if "last_recommendations" in st.session_state:
        final_recommendations = final_recommendations[~final_recommendations['id'].isin(st.session_state.last_recommendations)]

    # ✅ Step 10: Store the Latest Recommendations in Session State
    st.session_state.last_recommendations = final_recommendations['id'].tolist()

    # ✅ Step 11: Apply Diversity/Exploration if Needed
    if len(final_recommendations) < total_recommendations:
        exploration_videos = video_data[~video_data['id'].isin(final_recommendations['id'])].nlargest(total_recommendations - len(final_recommendations), 'recommend_score')
        exploration_videos['reason'] = 'Exploration'
        final_recommendations = pd.concat([final_recommendations, exploration_videos])

    # ✅ Step 12: Apply Language Filtering to Match User's Preferred Language
    final_recommendations = filter_language_recommendations(user_id, final_recommendations, total_recommendations)

    # ✅ Step 13: Ensure 25 Final Recommendations and Drop Duplicates
