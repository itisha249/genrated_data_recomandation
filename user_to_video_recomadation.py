...
def get_user_recommendations(user_id):
    ...
    if user_row.empty or (user_row[category_columns].sum().sum() == 0):
        ...
        if category_recommendations:
            top_recommendations = pd.concat(category_recommendations).sample(frac=1, random_state=42).reset_index(drop=True)
    ...
    if recommendations:
        top_recommendations = pd.concat(recommendations).sample(frac=1, random_state=42).reset_index(drop=True)
